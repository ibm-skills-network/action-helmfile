...
RELEASE_NAMESPACE="ibm-object-s3fs"
usage() {
...
helm2_install_usage() {
cat << EOF
This command installs a chart archive in IBM K8S Service(IKS).
...
helm_upgrade_usage() {
cat << EOF
This command upgrades a release to a new version of a chart in IBM K8S Service(IKS).
...
do
   ...
   case $key in
        ...
        if [[ $# -gt 1 ]] && [[ "$2" != "-"* ]]; then
          ...
          elif [[ "$2" = *"quotaLimit"* ]]; then
            CONFIG_QUOTA_LIMIT=$(echo $2 | cut -d "=" -f 2);
          elif [[ "$2" = *"allowCrossNsSecret"* ]]; then
            ALLOW_CROSS_NS_SECRET=$(echo $2 | cut -d "=" -f 2);
          else
            FLAGS+=("$1" "$2")
          fi
          shift
...
# Check cluster provider (RHOCP or IBM-Cloud)
# check if vpc cluster
...
# check if classic cluster
elif [[ `kubectl get nodes -o yaml | grep 'ibm-cloud\.kubernetes\.io/iaas-provider\: softlayer'` != "" ]]; then
    CLUSTER_PROVIDER="IBMC"
# check if rhocp cluster
elif [[ `kubectl get nodes -o yaml | grep 'node\.openshift\.io/os_id\: rhcos'` != "" ]]; then
    IS_IBM_CLOUD="false"
    CLUSTER_PROVIDER="RHOCP"
# check if satellite cluster
elif [[ `kubectl get nodes -o yaml | grep 'ibm-cloud\.kubernetes\.io/iaas-provider\: upi'` != "" ]]; then
    IS_IBM_CLOUD="false"
    CLUSTER_PROVIDER="Satellite"
    echo "ERROR: Satellite clusters are not supported via ibmc plugin. Use helm install command."
    exit 1
else
...
if [[ "$WORKER_OS" == "redhat" ]]; then
  echo "Fetching WORKER OS details ..."
  if [[ `kubectl get nodes -o yaml | grep 'node\.openshift\.io/os_id\: rhcos'` != "" ]];then
    KUBE_DRIVER_PATH="/etc/kubernetes"
  fi
fi

if [[ "$CONFIG_BUCKET_ACCESS_POLICY" == "true" ]]; then
...
if [ "$COMMAND" == "install" ]; then
  ...
  echo "CONFIG_QUOTA_LIMIT: "${CONFIG_QUOTA_LIMIT}
  echo "ALLOW_CROSS_NS_SECRET: "${ALLOW_CROSS_NS_SECRET}
  if [[ "$CLUSTER_PROVIDER" == "IBMC"* ]]; then
    echo "DC: ${DC_NAME}"
    echo "Region: ${REGION}"
  fi
  ...
  if [[ "$RELEASE_NAMESPACE" != "kube-system" && "$RELEASE_NAMESPACE" != "default" ]]; then
    if [[ `kubectl get ns | grep ${RELEASE_NAMESPACE}` != "" ]]; then
      # Add check to not perform patch if labels are as expected
      kubectl patch namespace ${RELEASE_NAMESPACE}  --type merge -p '{"metadata": {"labels": {"pod-security.kubernetes.io/enforce": "privileged", "pod-security.kubernetes.io/enforce-version": "latest", "pod-security.kubernetes.io/warn": "privileged", "pod-security.kubernetes.io/warn-version": "latest", "pod-security.kubernetes.io/audit": "privileged", "pod-security.kubernetes.io/audit-version": "latest"}}}'
    else
      kubectl apply -f - <<EOF
      kind: Namespace
      apiVersion: v1
...
EOF
  ...
  set +e
  if [[ ! -z "$CHARTVERSION" ]]; then
    helm ${PASSTHRU[@]} --version "$CHARTVERSION" --namespace ${RELEASE_NAMESPACE} --set bucketAccessPolicy="${CONFIG_BUCKET_ACCESS_POLICY}" --set quotaLimit="${CONFIG_QUOTA_LIMIT}" --set allowCrossNsSecret="${ALLOW_CROSS_NS_SECRET}" --set dcname="${DC_NAME}" --set region=${REGION} --set provider="${CLUSTER_PROVIDER}" --set workerOS="${WORKER_OS}" --set platform="${PLATFORM}" --set kubeDriver="${KUBE_DRIVER_PATH}" "${FLAGS[@]}"
  else
    helm ${PASSTHRU[@]} --namespace ${RELEASE_NAMESPACE} --set dcname="${DC_NAME}" --set region=${REGION} --set bucketAccessPolicy="${CONFIG_BUCKET_ACCESS_POLICY}" --set quotaLimit="${CONFIG_QUOTA_LIMIT}" --set allowCrossNsSecret="${ALLOW_CROSS_NS_SECRET}" --set provider="${CLUSTER_PROVIDER}" --set workerOS="${WORKER_OS}" --set platform="${PLATFORM}" --set kubeDriver="${KUBE_DRIVER_PATH}" "${FLAGS[@]}"
  fi
  exit_status=$(echo $?)
  ...
  if [[ "${FLAGS[@]}" = *"--install"* && "$RELEASE_NAMESPACE" != "kube-system" && "$RELEASE_NAMESPACE" != "default" ]]; then
    if [[ `kubectl get ns | grep ${RELEASE_NAMESPACE}` != "" ]]; then
      if [[ `helm ls -n ${RELEASE_NAMESPACE} | grep object-storage-plugin` == "" ]]; then
        kubectl patch namespace ${RELEASE_NAMESPACE}  --type merge -p '{"metadata": {"labels": {"pod-security.kubernetes.io/enforce": "privileged", "pod-security.kubernetes.io/enforce-version": "latest", "pod-security.kubernetes.io/warn": "privileged", "pod-security.kubernetes.io/warn-version": "latest", "pod-security.kubernetes.io/audit": "privileged", "pod-security.kubernetes.io/audit-version": "latest"}}}'
      fi
    else
      kubectl apply -f - <<EOF
...
EOF
  ...
  set +e
  if [[ ! -z "$CHARTVERSION" ]]; then
    helm ${PASSTHRU[@]} --version "$CHARTVERSION" --namespace ${RELEASE_NAMESPACE} --set bucketAccessPolicy="${CONFIG_BUCKET_ACCESS_POLICY}" --set quotaLimit="${CONFIG_QUOTA_LIMIT}" --set allowCrossNsSecret="${ALLOW_CROSS_NS_SECRET}" --set region=${REGION} --set dcname="${DC_NAME}" --set provider="${CLUSTER_PROVIDER}" --set workerOS="${WORKER_OS}" --set platform="${PLATFORM}" --set kubeDriver="${KUBE_DRIVER_PATH}" "${FLAGS[@]}"
  else
    helm ${PASSTHRU[@]} --namespace ${RELEASE_NAMESPACE} --set dcname="${DC_NAME}" --set bucketAccessPolicy="${CONFIG_BUCKET_ACCESS_POLICY}" --set quotaLimit="${CONFIG_QUOTA_LIMIT}" --set allowCrossNsSecret="${ALLOW_CROSS_NS_SECRET}" --set region=${REGION} --set provider="${CLUSTER_PROVIDER}" --set workerOS="${WORKER_OS}" --set platform="${PLATFORM}" --set kubeDriver="${KUBE_DRIVER_PATH}" "${FLAGS[@]}"
  fi
  exit_status=$(echo $?)
  if [[ $exit_status -ne 0 ]]; then
    if [[ $flag -eq 1 ]]; then
